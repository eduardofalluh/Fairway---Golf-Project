...
export function Nav() {
  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${scrolled ? "border-b border-line/80 bg-base/94 text-cream shadow-[0_8px_30px_rgba(22,45,34,.08)] backdrop-blur-xl" : "border-b border-white/15 bg-forest/35 text-white backdrop-blur-sm"}`}>
      <nav className="mx-auto flex h-16 max-w-[1440px] items-center justify-between gap-3 px-4 transition-[height] duration-300 sm:h-[78px] sm:px-8 lg:px-10">
        <a href="#" className="flex shrink-0 items-center gap-2 font-display text-[1.65rem] font-semibold tracking-tight sm:gap-2.5 sm:text-2xl">
        ...
        <a href="#search" className={`group inline-flex min-h-11 shrink-0 items-center gap-2 rounded-full px-4 py-2 text-xs font-bold uppercase tracking-[0.12em] shadow-[0_12px_28px_rgba(0,0,0,.14)] transition sm:py-2.5 ${scrolled ? "bg-forest text-white hover:bg-forest-soft" : "bg-lime text-forest hover:bg-white"}`}>
